...
<div style="display:flex; gap:16px;" class="mb-2">
    ...
    <div><a class="btn btn-secondary" href="/page/users?title=<?= $_GET["title"] ?>&include=1" role="button">All</a></div>
</div>
...
?>
        <div class="row mb-2 ms-1" style="border:solid 1px; border-left:10px solid white; text-align:center" >
            ...
            <div class="col">
                <div class="py-1 <?= $colorRed ?>" style="font-size:20px;"><div style="font-weight:bold;"><?= $name.($r['type']=='admin'?' (Admin)':'') ?></div> <div class="text-info" style="overflow:auto;"><?= $mail ?></div> </div>
                ...
                <div style="border-top: solid 1px white;" class="py-1">
                    ...
                    <?php if(($_GET["title"]=="Friends") || $imAdmin) { ?>
                        <?php if(($_SESSION["blocked"]==0 && $is_friend_not_blocking_me) || $imAdmin) { ?>
                            <a class="btn btn-primary" href="/page/create_post?id_r=<?=$r["id"]?>&name_r=<?=$username?>">Post</a> | 
                        <?php }else{ ?>
                    ...
                    <?php if($_GET["title"]=="Friends" || $imAdmin) { ?>
                        <a class="btn btn-success" href="/page/posts?name=<?=$username?>">All Posts</a> | 
                    <?php } ?>
                    
                    <?php if(($_GET["title"]=="Friends") || $imAdmin) { ?>
                        <?php if(($_SESSION["blocked"]==0 && $is_friend_not_blocking_me) || $imAdmin) { ?>
                            <a class="btn btn-warning" href="/page/gallery?user=<?= $r["id"] ?>">Gallery</a> | 
                        <?php }else{ ?>
                    ...
                    <?php if($_GET["title"]=="Users" && $imAdmin) { ?>
                        <?php if($r["blocked"]==1) { ?>
                            <a href="/unblockuser?unblockuser=<?= $r["id"] ?>&title=<?= $_GET["title"] ?>" class="btn" style="background-color:purple; color:white;">UnBlock</a> | 
                        <?php }else{ ?>
                            <a href="/blockuser?blockuser=<?= $r["id"] ?>&title=<?= $_GET["title"] ?>" class="btn" style="background-color:purple; color:white;">Block</a> | 
                        <?php } ?>
                    <?php } else if($_GET["title"]=="Friends") { ?>
                        <?php if(isset($MyfriendsArray) && $MyfriendsArray[$r["id"]]==1) { ?>
                            <a href="/unblockfriend?unblockfriend=<?= $r["id"] ?>&title=<?= $_GET["title"] ?>" class="btn" style="background-color:purple; color:white;">UnBlock</a> | 
                        <?php }else{ ?>
                            <a href="/blockfriend?blockfriend=<?= $r["id"] ?>&title=<?= $_GET["title"] ?>" class="btn" style="background-color:purple; color:white;">Block</a> | 
                        <?php } ?>
                    <?php } ?>

                    <?php if(isset($MyfriendsArray[$r["id"]])) { ?>
                        <a href="/unfriend?unfriend=<?= $r["id"] ?>&title=<?= $_GET["title"] ?>&img=<?= $r["img"] ?>" class="btn btn-danger" onclick="return confirm('Remove from Friends list ?');">Unfriend</a>
                    <?php }else{ ?>
                        <a href="/befriend?befriend=<?= $r["id"] ?>&title=<?= $_GET["title"] ?>" class="btn btn-success">Befriend</a>
                    <?php } ?>
