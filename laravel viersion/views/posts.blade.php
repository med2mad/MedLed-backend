...
<?php
    ...
    $name = isset($_GET["name"]) ? trim($_GET["name"]):''; //if search happend
?>
...
<div style="display:flex; gap:16px;" class="mb-2">
    ...
    <div><a class="btn btn-secondary" href="/page/posts?include=1" role="button">All</a></div>
</div>
...
<p style="text-align:center; background-color:rgb(60,60,60); border:solid 1px white; font-size:20px">
    <a class="text-primary" href="/page/posts?page=1&name=<?= $name ?>"> &nbsp; << &nbsp; </a> <?php
    for ($i=1; $i<=$pagesnbr; $i++){ 
        if($currantpage!=$i){
        ?><a class="text-primary" href="/page/posts?page=<?= $i ?>&name=<?= $name ?>"> &nbsp; <?= $i ?> &nbsp; </a>
        <?php }else{ ?>
        &nbsp; <?= $i ?> &nbsp; 
    <?php }}?> 
    <a class="text-primary" href="/page/posts?page=<?= $pagesnbr ?>&name=<?= $name ?>"> &nbsp; >> &nbsp; </a>
</p>
<?php
    include ("conn.blade.php");
    $q = "select * from posts";
    ...
    <table class="table table-bordered mb-5" cellspacing="0" style="font-size:1em;" >
        ...
        <tr> 
            <td colspan="4" style="border: solid 1px; border-left: 10px solid white;">
                <a class="btn btn-success" href="/page/post_view?id=<?= $r["id"] ?>">View</a> | 
                <?php if($r["users_id_w"]!=$_SESSION["id"]){ 
                    $name_w = urlencode($r["users_name_w"]);?>
                    <a class="btn btn-primary" href="/page/create_post?id_r=<?=$r["users_id_w"]?>&name_r=<?=$name_w?>">Respond</a> | 
                <?php } ?>
                <a class="btn btn-danger" href="/deletepost?deletemessage=<?= $r["id"] ?>&file=<?= $r["file"] ?>" onclick="return confirm('Delete this post ?');">Delete</a>
            </td>
        </tr>
