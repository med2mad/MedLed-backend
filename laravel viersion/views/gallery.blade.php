...
<div style="margin:0px; padding:0px; text-align:right">
    <div class="col"> - <a class="text-primary" href="/page/create_gallery">Add to Gallery</a></div>
</div>

<p class="pagenum" >
    <a class="text-primary" href="/page/gallery?user=<?= $_GET["user"] ?>&page=1&perpage=<?= $perpage ?><?= $date1.$date2 ?>#ppp"> &nbsp; << &nbsp; </a> 
                ...
                <div class="col">
                    <div>
                        ...
                        <?php if(isset($_SESSION["id"]) && $_SESSION["id"]==$_GET["user"] || isset($_SESSION["type"]) && $_SESSION["type"]=="admin"){ ?>
                            <p><a class="btn btn-danger" href="/deletegallery?deletegallery=<?= $r["id"] ?>&user=<?= $_SESSION["id"] ?>&img=<?= $r["img"] ?>&page=<?= $currentpage ?>&perpage=<?= $perpage ?>" onclick="return confirm('Delete this image ?');">Delete</a></p>
                        <?php } ?>
                    </div>
                </div>
    <?php   } 
