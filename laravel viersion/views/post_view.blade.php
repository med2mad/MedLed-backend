        ...
        <div style="margin:15px; padding:15px;">
            <p style="text-align:center;">
                <?php if($r["users_id_w"]!=$_SESSION["id"]){ ?>
                    <a class="btn btn-primary" href="/page/create_post?id=<?=$r["users_id_w"]?>&name=<?= urlencode($r["users_name_w"]) ?>&mail=<?= urlencode($r["users_mail_w"]) ?>&img=<?=$r["users_img_w"]?>">Respond</a> | 
                <?php } ?>
                <a class="btn btn-danger" href="/deletepost?deletemessage=<?= $r["id"] ?>" onclick="return confirm('Delete this post ?');">Delete</a>
            </p>
        </div>
    <?php } ?>
